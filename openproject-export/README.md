...
```
$ cd $OPENPROJECT_ROOT
$ RAILS_ENV=test bundle exec rspec `bundle show openproject-export`/spec/controllers/kittens_controller_spec.rb
```
...
A plugin's seeders have to be defined under its namespace within the `BasicData` module, for instance `BasicData::Export::KittensSeeder`.
...
* `lib/open_project/export/engine.rb` - assets statement at the end of the engine.
* `app/assets/javascripts/export/main.js` - main entry point for plain JavaScript and document ready hook.
* `app/assets/stylesheets/export/main.scss` - good ol' Sass stuff.

Any additional assets you want to use have to be registered for pre-compilation in the engine like this:

```
assets %w(your_image.png)
```

You don't technically have to put the assets into a subfolder with the same name as your plugin. But it's highly recommended to do so in order to avoid naming conflicts. For example, if the image `your_image.png` is not scoped, it might conflict with the core if it were also to include another asset named the same.
...
* `lib/open_project/export/engine.rb` - register block in the beginning
...
menu :project_menu, # Which menu to add an item to (compare the core config/initializers/menus.rb for options)
     ...
     caption: :"export_plugin_name", # The caption, use a symbol for I18n lookup, or a string for plain text
